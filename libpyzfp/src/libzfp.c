...
usage()
{
  ...
  fprintf(stderr, "  -b <pointer to array> : reference array for passing in from cython\n"); // added by spm
...
int compress(int argc, char* args[], void* carray) // modified by spm
{
  // Added by spm to set array operational mode
  int arrayMode=0; // 0/1 out to cython/in from cython added by spm
  ...
  // Modified by spm to utilize passed parameter list beginning at '0'
  ...
  for (i = 0; i < argc; i++) {
    ...
    switch (args[i][1]) {
      case '1':
        if (++i == argc || sscanf(args[i], "%u", &nx) != 1)
          usage();
        ny = nz = 1;
        dims = 1;
        break;
      ...
      case '3':
        if (++i == argc || sscanf(args[i], "%u", &nx) != 1 ||
            ++i == argc || sscanf(args[i], "%u", &ny) != 1 ||
            ++i == argc || sscanf(args[i], "%u", &nz) != 1)
          usage();
        dims = 3;
        break;
      case 'a':
        if (++i == argc || sscanf(args[i], "%lf", &tolerance) != 1)
          usage();
        mode = 'a';
        break;
      case 'c':
        if (++i == argc || sscanf(args[i], "%u", &minbits) != 1 ||
            ++i == argc || sscanf(args[i], "%u", &maxbits) != 1 ||
            ++i == argc || sscanf(args[i], "%u", &maxprec) != 1 ||
            ++i == argc || sscanf(args[i], "%d", &minexp) != 1)
          usage();
        mode = 'c';
        break;
      case 'd':
      ...
      case 'o':
        ...
        break;
      case 'b': // Added by spm
        if (++i == argc || sscanf(args[i], "%u", &arrayMode) != 1)
          usage();
        break;
      case 'p':
        if (++i == argc || sscanf(args[i], "%u", &precision) != 1)
          usage();
        mode = 'p';
        break;
      case 'q':
        ...
        break;
      case 'r':
        if (++i == argc || sscanf(args[i], "%lf", &rate) != 1)
          usage();
        mode = 'r';
        break;
      case 's':
  ...
  // Added by spm to ensure infile and passed array not simultaneous
  if (arrayMode && inpath )  {
      fprintf(stderr, "must specify input file via (-i or -z) or pass array with -b but not both\n");
      return EXIT_FAILURE;
  }

  // Modified by spm to allow for passed array
  /* make sure we have an input file or passed array */
  if (!arrayMode && !zfppath )  {
        if (!inpath && !zfppath)  {
          fprintf(stderr, "must specify uncompressed or compressed input file via -i or -z or pass array with -b or -g\n");
          return EXIT_FAILURE;
        }
  }
  ...
  //Modified by spm to allow for stat'ing with an array
  /* make sure we have input file for stats */
  if (stats && !inpath && !arrayMode)  {
    fprintf(stderr, "must specify input file via -i or pass array IN via '-b 1' to compute stats\n");
    return EXIT_FAILURE;
  }

  /* make sure meta data comes from header or command line, not both */
  /* Added array mode operations spm */
  if (!inpath && !arrayMode && zfppath && header && (typesize || dims)) {
  ...
  // 'if' logic Modified by spm to allow for passed array
  /* read uncompressed or compressed file or passed array */
  if (inpath) {
  ...
  // Modified by spm to allow for passed array
  /* compress input file or passed array if provided */
  if (inpath || arrayMode) {
  ...
  if ((!inpath && zfppath) || outpath || stats) {
    ...
    // Added by spm to copy out data from decompress to passed array
    if(!arrayMode) {
      memcpy(carray,fo, rawsize);
    }
  ...
  if (!quiet) {
    ...
    if (stats) {
      if(!carray) {
        ...
        print_error(carray, fo, type, nx * ny * nz);
      }
    }
    fprintf(stderr, "\n");
  }
